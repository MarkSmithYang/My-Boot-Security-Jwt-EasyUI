...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws ServletException, IOException {
        //从redis里获取token来判断请求的合法性,因为是前后端未分离的系统,这里就不去请求头获取token判断了
        String ipAddress = RealIpGetUtils.getIpAddress(request);
        String token = (String) redisTemplate.opsForValue().get(CommonDic.LOGIN_SUCCESS_TOKEN + ipAddress);
        //获取请求头里的token值----->因为是前后端在一起,所以就不去请求头获取token了
        ...
        if (CollectionUtils.isNotEmpty(detailsInfo.getRoles())) {
            Set<SimpleGrantedAuthority> collect = detailsInfo.getRoles().stream().map(s -> {
                return new SimpleGrantedAuthority(CommonDic.ROLE_ + s);
            }).collect(Collectors.toSet());
            //合并集合数据
            authorities.addAll(collect);
        }
        ...
        if (CollectionUtils.isNotEmpty(detailsInfo.getModules())) {
            Set<SimpleGrantedAuthority> collect = detailsInfo.getModules().stream().map(a -> {
                return new SimpleGrantedAuthority(CommonDic.MODULE_ + a);
            }).collect(Collectors.toSet());
            //合并集合数据
            authorities.addAll(collect);
        }
